...
DEG_out_dir <- glue("{out_dir_root}/04.downstream_analysis/DE_random_effect_C2L_0116")
...
comparison <- c("C9orf72_vs_Control")
p_thresh <- 0.05
fc_thresh <- log2(1.5)
...
out_data_dir <- glue("{DEG_out_dir}/{comparison}/data")
out_plot_dir <- glue("{DEG_out_dir}/{comparison}/plot")
...
cur_enriched_spot_ids <- dplyr::filter(s@meta.data, get(cluster) == "Enriched")|>
  rownames()
clust_s <- subset(s, cells = cur_enriched_spot_ids)
...
genes_keep_10pct <- row.names(LFC)[LFC$pct.1 >= minimal_dom_exp | LFC$pct.2 >= minimal_dom_exp]
genes_keep_1pct <- row.names(LFC)[LFC$pct.1 >= minimal_exp & LFC$pct.2 >= minimal_exp]
...
tryCatch(
{
  ...
  # TODO: change to condition when applicable
  zlm_condition <- zlm(~ condition + cdr_centered + (1 | sample_id), sca,
                       ebayes = FALSE, method = "glmer")

  # TODO: change to condition or condition general
  lrt_name <- paste0("condition", ident.1)
