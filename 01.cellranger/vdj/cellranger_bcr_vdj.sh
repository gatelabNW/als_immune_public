...
# 02/08/2023
####################################################################
# Run 01.cellranger vdj on BCR fastq files from ALS project
...
# Execute "bash batch_cellranger_bcr_vdj.sh"
####################################################################

date

# Define and check directory containing GEX fastq files
fastq_dir="/projects/b1042/Gate_Lab/als/scrna-original/raw/raw_data"
[ -d "$fastq_dir" ] && echo "Directory to fastq files exists."

# Define, make (if necessary), and navigate to output directory
output_dir="/projects/b1042/Gate_Lab/projects/als-project/scrna_original_final/01.cellranger_bcr_vdj"
[ -d "$output_dir" ] || mkdir "$output_dir"

# Create array of fastq_dir subdirectories

find "$fastq_dir" -maxdepth 1 -mindepth 1 -type d -name "*BCR*" -print0 | while read -d $'\0' sample_dir
do
...
#  fi
  ...
  sbatch cellranger_vdj.sh "$id" "$sample_dir" "$sample" "$output_dir"
