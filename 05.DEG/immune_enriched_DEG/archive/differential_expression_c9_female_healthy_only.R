...
degs_path<-glue("{DEG_out_dir}/degs/female_c9_hc")
plots_path<-glue("{DEG_out_dir}/plots/female_c9_hc")
  ...
  comparisons |>lapply(function(comparison) {
    ...
    if(compA_cellnum<10 | compB_cellnum<10){
      ...
      for(cur_cov_str in all_cov){
        ...
        degs <- find_degs(s,
                          save_path = glue("{degs_path}/{cur_cov_str}/female_c9_hc___{filename}.csv"),
        ...
        volcano_plot(degs,
                     file = glue("{plots_path}/{cur_cov_str}/female_c9_hc___{filename}.pdf"),
                     title = title,
