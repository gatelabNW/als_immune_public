...
# Expected output:
#     seurat_merged_001.rds: a seurat object before any QC is perfomed on
#     seurat_merged_002.rds: a seurat object with only singlets, MT<10
####################################################################
...
samples <- samples[samples != "Undetermined"]

# NOTE: Correcting for mislabeled samples
...
add_cell_ids <- sapply(samples, function(x){
  new_sample <- name_mapping[[x]]
  print(glue("{x} named to {new_sample}!"))
  new_sample
})

seurat_object_list <- samples |> lapply(\(sample) {
  # get the most updated sample number
  corrected_sample <- name_mapping[[sample]]
  CreateSeuratObject(
    counts = "{soupX_count_mtx_root}/{sample}" |> glue() |> Read10X(),
    project =  corrected_sample,
    min.cells = 3,
    min.features = 200
  )
})
