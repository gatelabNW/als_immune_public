...
raw_root="/projects/b1042/Gate_Lab/benney/Long_Read_Analysis"

while IFS= read -r dir; do
    # Your code to handle each directory goes here
    cur_out_dir=$(echo "$dir" | sed 's/pod5/Basecall_Output/')
...
done < <(find "$raw_root" -type d -name "*pod5" -exec realpath {} \;)